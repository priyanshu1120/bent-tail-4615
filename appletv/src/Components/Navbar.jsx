...
const Navbar = () => {
  ...
  return (
    <>
      <div >
        <Box zIndex={1000} position="fixed"p={0}  top={0} left={0} w={"100%"}
             bg={useColorModeValue('black', 'gray.600')} px={10} as="header"  >
          ...
          <IconButton size={'md'}  icon={isOpen ? <CloseIcon /> : <HamburgerIcon />}
            ...
            <HStack spacing={8} alignItems={'center'}>
              <Box>
                <NavLink to="/">
                  <Image overflow={"hidden"} h={10} pl={[50,null,null]} w={"auto"} src={Weblogo} alt='LOGO' />
                </NavLink>
              </Box>
              {onOpen ? (
              <Box  display={{ md: 'none' }}>
                <Stack as={'nav'} spacing={4}> 
                  <Flex justify={"center"} align={"center"}> 
                      {avatar?"":
                        <HStack >
                          <Link to="/signup">
                            <Button leftIcon={<MdOutlineManageAccounts />} colorScheme='twitter' size='md' variant='solid'>
                              Sign Up
                            </Button>
                          </Link>
                        </HStack>} 
              ...
              <HStack as={'nav'} spacing={2} display={{ base: 'none', md: 'flex' }}>
                  ...
                  <InputGroup >
                    <InputLeftElement pointerEvents='none' children={<SearchIcon/>} /> 
                      <Input color={"whiteAlpha.900"} value={ SearchInputText}  onChange={handelQuery}  
                             type='text' htmlSize={12} width="auto" placeholder='Search' />
                  </InputGroup> 

                      {avatar?""
                  :<InputGroup>
                      <Link >
                        <Button onClick={onOpen} leftIcon={<MdOutlineManageAccounts />} colorScheme='twitter' variant='solid'>
                          Sign up
                        </Button>
                      </Link>
                    </InputGroup>}
          ...
          {isOpen ? (
            <Box pb={9} display={{ md: 'none' }}>
              <Stack as={'nav'} spacing={4}>
                <Flex pl={9} justify={"center"} align={"center"}>
