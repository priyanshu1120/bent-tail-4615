  ...
  >
    { data?.map( slide => (
     ...
     <div style={{borderRadius:"7px"}}  key={slide.id} onClick={ ()=>  handleclick(slide.id)} >
      <img  src={slide.image}  style={{height:"100%" ,borderRadius:"7px"}}/>
      <Text fontSize={"13px"}>Episode {slide.episode}</Text>
      <Text fontSize={"14px"} fontWeight="bold" w={"200px"}>{slide.title}</Text>
      <Text fontSize={"13px"} w={"200px"}>{slide.description}</Text>
        <Text fontSize={"13px"} mt={"10px"}>{slide.time}  {slide.date}</Text>
     </div>
